...
function verifyJWT(req, res) {
  ...
  // デバッグ情報を追加
  console.log(
    `[${new Date().toISOString()}] JWT認証リクエスト: ${req.method} ${req.url}`
  );
  console.log(
    `[JWT] トークン: ${token ? token.substring(0, 20) + "..." : "なし"}`
  );
  ...
  try {
    ...
    req.user = decoded;
    // Store token expiration time (convert from Unix timestamp if needed)
    req.tokenExpiration = decoded.exp ? new Date(decoded.exp * 1000) : null;
    console.log(
      `[JWT] 認証成功: ユーザー ${JSON.stringify(decoded)}, 有効期限: ${
        req.tokenExpiration
      }`
    );
    return true;
...
async function Write(req, res, HTTP) {
  ...
  await fs.promises.writeFile(
    __dirname + decodeURIComponent(req.headers["write"]),
    body
  );
...
global.serve = async (PORT) => {
  ...
  HTTP.on("request", (req, res) => {
    ...
    if (remoteIp !== global.address) {
      ...
      console.log(
        `[Auth Bypass] 自身のIP (${remoteIp}) からのリクエストなので認証をスキップ`
      );
    }
  ...
  HTTP.on("upgrade", (req, socket, head) => {
    ...
    const isUnrealConnection =
      protocol !== "peer-stream" && protocol !== "exec-ue";
    ...
    if (!shouldBypass) {
      if (global["jwt-auth"]) {
        ...
        console.log(
          `[${new Date().toISOString()}] WS接続リクエスト: ${req.url}`
        );
        ...
        try {
          ...
          req.tokenExpiration = decoded.exp
            ...
            : null;
          console.log(
            `[WS-JWT] 認証成功: ユーザー ${JSON.stringify(
              decoded
            )}, 有効期限: ${req.tokenExpiration}`
          );
        } catch (error) {
      ...
      console.log(
        `[WS Auth Bypass] Unrealエンジン接続かつ許可IPからのリクエスト (${remoteIp}) なので認証をスキップ`
      );
    }

    // WS子プロトコルに応じたアップグレード処理
    if (protocol === "peer-stream") {
      PLAYER.handleUpgrade(req, socket, head, (ws) => {
        if (req.tokenExpiration) {
          ws.tokenExpiration = req.tokenExpiration;
        }
        PLAYER.emit("connection", ws, req);
      });
    } else if (protocol === "exec-ue") {
      EXECUE.handleUpgrade(req, socket, head, (ws) => {
        if (req.tokenExpiration) {
          ws.tokenExpiration = req.tokenExpiration;
        }
        EXECUE.emit("connection", ws, req);
      });
    } else {
      ENGINE.handleUpgrade(req, socket, head, (ws) => {
        if (req.tokenExpiration) {
          ws.tokenExpiration = req.tokenExpiration;
        }
        ENGINE.emit("connection", ws, req);
      });
    }
  });
...
global.killPlayer = async function (playerId) {
  const fe = [...PLAYER.clients].find(
    (a) => a.req.socket.remotePort === playerId
  );
